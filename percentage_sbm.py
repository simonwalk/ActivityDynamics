...
init_name = "SBM_SAME_CONN_MEDIUM"
...
ratios = [1, 10, 30, 80, 100, 120]#, 160, 200, 240, 280, 320, 360]
...
nodes_per_block = [x*100 for x in xrange(1, 11,1)]
num_blocks = 10
...
def create_network():
    ...
    bg.debug_msg("Creating BlockModel network!")

    bg.create_stochastic_blockmodel_graph(blocks=num_blocks, size=nodes_per_block,
                                          self_block_connectivity=self_block, other_block_connectivity=other_block,
                                          power_exp=power_exp)
    bg.debug_msg("BlockModel with {} nodes and {} edges!".format(bg.graph.num_vertices(), bg.graph.num_edges()))
