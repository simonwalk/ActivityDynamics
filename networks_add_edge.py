...
def create_network(graph_name, network, strategy):
    ...
    num_array = nw.get_num_edges_per_iterations_array(nw.maximum_edges_of_graph(), num_iterations)
    debug_msg("Starting with strategy: " + strategy)
    for i in range(1, num_iterations + 1):
        debug_msg("Starting with iteration: " + str(i))
        nw.add_edge_to_graph(num_array[i-1], strategy)
...
if __name__ == '__main__':
    for network in networks_used:
        base_graph = network + " Network Add Edge "
