...
cex_paper = 1.5
cex_size = 0.6
colors = c("#000000", "#858585", rgb(0, 0, 0, 0.8), rgb(0, 1, 0, 0.8))
...
par(mar=c(5,5,4,5)+.1)
plot(head(data$real_act_x, -1), head(data$ratios, -1), type="o", lwd=line_width, pch=4, col=colors[1], xlab=xlabel, ylab="Ratio", cex.axis=cex_paper, cex.lab=cex_paper, ylim=c(min_y, max_y))
lines(head(data$real_act_x, -1), head(data$k1s, -1), type="o", lwd=line_width, pch=1, lty=2, col=colors[2])
...
pdf("ratio_legend.pdf", width=14, height=1)
par(mar=c(0,0,0,0))
plot.new()
...
par(mar=c(5,5,4,5)+.1)
plot(data$real_act_x, data$cor_sim_act_y, type="o", lwd=line_width, pch=4, xlab=xlabel, ylab="Activity", lty=1, col=colors[1], cex=cex_size, cex.axis=cex_paper, cex.lab=cex_paper, ylim=c(min_y, max_y))
lines(data$real_act_x, data$real_act_y, type="o", lwd=line_width, lty=1, pch=1, col=colors[2])
i = 1
while (i < length(data$real_act_x)) {
    lines(c(i-1, i), c(data$cor_sim_act_y[i], data$sim_act_y[i+1]), lwd=line_width, lty=2, col=colors[3])
    lines(c(i, i), c(data$sim_act_y[i+1], data$cor_sim_act_y[i+1]), lwd=line_width, lty=2, col=colors[4])
    i = i + 1
}
...
pdf("activity_legend.pdf", width=14, height=1)
par(mar=c(0,0,0,0))
plot.new()
legend("center", pch=c(4,1,NA,NA), col=colors, legend=c("Adjusted Simulated Activity", "Empirical Activity", "Simulated Activity", "Activity Gain"), lty=c(1,1, 2, 2), lwd=1, cex=1, ncol=4)
dev.off
...
par(mar=c(5,5,4,5)+.1)
plot(errors_x, errors, type="o", lwd=line_width, xlab=xlabel, ylab="Activity", lty=1, pch=4, col=colors[1], cex.axis=cex_paper, cex.lab=cex_paper)
