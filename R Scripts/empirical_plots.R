...
lines(head(k1_x, -1), head(k1_y, -1), type="o", lwd=line_width, pch=1, lty=2, col=colors[2])
...
par(mar=c(5,5,4,5)+.1)
plot(data$real_act_x, data$sim_act_y, type="l", pch=4, xlab=xlabel, ylab="Activity", lty=1, col=colors[1], cex=cex_size, cex.axis=cex_paper, cex.lab=cex_paper, ylim=c(min_y, max_y))
...
title(substitute(atop("Error of Simulation over " ~ tau ~ " (in " * mode * ")",
                      "(RMSE = " ~ rmse ~ ")"), list(rmse=round(rmse,2), mode=mode)), cex.main=cex_paper)
grid(col="gray", lwd=1)
dev.off()