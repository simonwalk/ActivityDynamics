...
ratio_counter = 1
pdf("../Average_Activity_per_NW.pdf")
ita = 0
for(ratio in ratio_strings){
  ...
  for(fp in filepaths) {
    ...
    if(ita==0){
      t <- bquote("Average Activity per Network at" ~ tau ~ " over all " ~ frac(lambda,mu))
      par(mar=c(5.1, 4.1, 4.1, 8.1))
      plot(x_val, y_val, xlim=c(0, max_x), ylim=c(0, max_y), type="l", pch=1, col=colors[ita+1],
           xlab=expression(tau), ylab="Average Activity per Network", lty=ita+1, 
           main=t)
      ita = ita +1
    } else {
      #print(y_val)
      print(ita)
      lines(x_val, y_val, type="l", pch=ita, col=colors[ita], lty=ita)
    }
    ita = ita+1
    if(ita > 5)
      ita = 1
